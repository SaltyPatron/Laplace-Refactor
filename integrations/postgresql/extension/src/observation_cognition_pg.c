...
static Datum* observation_deconstruct_composite_array(
    ...
    if (allow_empty && ARR_NDIM(array) == 0) {
        *count = 0;
        *nulls = NULL;
        return NULL;
    }
...
static laplace_persistence_trajectory_segment_record* observation_read_segments(
    ...
    for (index = 0; index < count; ++index) {
        ...
        int64 metadata_signed;
        int64 atom_signed;
        int32 packed_ordinal;
        int32 run_length;
        int16 tier;
        bool has_atom;
        ...
        if (atom_signed < 0 || (uint64_t)atom_signed > UINT32_MAX ||
            packed_ordinal < 0 || packed_ordinal > (int32)UINT16_MAX ||
            run_length <= 0 || run_length > (int32)UINT16_MAX ||
            tier < 0 || tier > (int16)UINT8_MAX) {
            ereport(ERROR,
                    (errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
