...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_catalog.pg_namespace AS n
         WHERE n.nspname = 'laplace' AND n.nspowner = extension_owner) THEN
        RAISE EXCEPTION 'Laplace schema is not owned by the extension owner';
    END IF;
    ...
    SELECT t.typrelid
     ...
     WHERE t.oid = type_oid
       ...
       AND t.typnamespace = pg_catalog.to_regnamespace('laplace')
       AND t.typtype = 'c';
    IF type_relid IS NULL OR type_relid = 0 THEN
        RAISE EXCEPTION 'cognition_realization_result is not the owned Laplace composite';
    END IF;
    ...
    IF names IS DISTINCT FROM ARRAY[
       ...
       OR types IS DISTINCT FROM ARRAY[
            ...
            'numeric(20,0)','integer','integer','integer','integer','integer','integer']::text[] THEN
    ...
    SELECT t.typrelid
     ...
     WHERE t.oid = type_oid
       ...
       AND t.typtype = 'c';
    IF type_relid IS NULL OR type_relid = 0 THEN
        RAISE EXCEPTION 'cognition_materialization_request is not the owned Laplace composite';
    END IF;
    SELECT pg_catalog.array_agg(a.attname::text ORDER BY a.attnum),
           pg_catalog.array_agg(pg_catalog.format_type(a.atttypid, a.atttypmod) ORDER BY a.attnum)
      INTO names, types
    ...
    IF names IS DISTINCT FROM ARRAY[
       ...
       OR types IS DISTINCT FROM ARRAY[
            'numeric(20,0)','numeric(20,0)','numeric(20,0)','integer','integer']::text[] THEN
    ...
    SELECT t.typrelid
     ...
     WHERE t.oid = type_oid
       ...
       AND t.typtype = 'c';
    IF type_relid IS NULL OR type_relid = 0 THEN
        RAISE EXCEPTION 'cognition_materialization_result is not the owned Laplace composite';
    END IF;
    SELECT pg_catalog.array_agg(a.attname::text ORDER BY a.attnum),
           pg_catalog.array_agg(pg_catalog.format_type(a.atttypid, a.atttypmod) ORDER BY a.attnum)
      INTO names, types
    ...
    IF names IS DISTINCT FROM ARRAY[
       ...
       OR types IS DISTINCT FROM ARRAY[
            ...
            'integer','integer','integer']::text[] THEN
    ...
    IF function_oid IS NOT NULL AND NOT EXISTS (
        SELECT 1
         ...
         WHERE p.oid = function_oid
           ...
           AND n.nspname = 'laplace'
           AND n.nspowner = extension_owner
           AND l.lanname = 'c'
