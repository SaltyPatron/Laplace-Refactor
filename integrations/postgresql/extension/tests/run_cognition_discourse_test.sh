...
test_root=$(mktemp -d "$temporary_parent/laplace-postgres-discourse.XXXXXX")
data_directory="$test_root/data"
socket_directory=$(mktemp -d /tmp/lp-discourse-pg.XXXXXX)
server_log="$test_root/postgres.log"
port=${LAPLACE_POSTGRES_DISCOURSE_TEST_PORT:-55443}
server_started=0
...
cleanup() {
    ...
    if [[ "$socket_directory" == /tmp/lp-discourse-pg.* ]]; then
...
stop_server() {
    "$pg_bindir/pg_ctl" -D "$data_directory" -m fast -t 10 -w stop >/dev/null
    server_started=0
}
...
if [[ ! -x "$native_probe" ]]; then
    echo "discourse persistence native oracle is unavailable" >&2
    exit 65
fi
probe_output=$(
...
while IFS='=' read -r key value; do
    ...
    if [[ ! "$key" =~ ^[A-Z0-9_]+$ || ! "$value" =~ ^[0-9a-f]+$ ]]; then
        echo "invalid discourse persistence oracle output: $key=$value" >&2
        exit 66
    fi
...
LD_PRELOAD="${sanitizer_preload}${sanitizer_preload:+${LD_PRELOAD:+:}}${LD_PRELOAD:-}" \
"$pg_bindir/psql" "${psql_arguments[@]}" -f "$deposit_sql"

# The durable boundary is only proved after a real postmaster restart. The next
# connection must reconstruct and validate the exact native frames from storage.
stop_server
start_server

LD_PRELOAD="${sanitizer_preload}${sanitizer_preload:+${LD_PRELOAD:+:}}${LD_PRELOAD:-}" \
"$pg_bindir/psql" "${psql_arguments[@]}" -f "$readback_sql"
