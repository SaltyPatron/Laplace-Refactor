...
constexpr std::size_t CoordinateWords = 16u;
...
bool AddWords(const std::uint64_t binding_count, std::size_t& required) {
    if (binding_count >
        static_cast<std::uint64_t>(
            (std::numeric_limits<std::size_t>::max() - HeaderWords) / BindingWords)) {
        return false;
    }
...
extern "C" laplace_universal_ast_status laplace_universal_ast_plan_packet_encode(
    ...
    *cursor++ = static_cast<std::uint32_t>(required);
    ...
    if (static_cast<std::size_t>(cursor - output_words) != required) {
        return LAPLACE_UNIVERSAL_AST_OVERFLOW;
    }
    return LAPLACE_UNIVERSAL_AST_OK;
}
...
extern "C" laplace_universal_ast_status laplace_universal_ast_packet_validate_words(
    ...
    if (DigestZero(receipt->grammar_registry_fingerprint) ||
        ...
        DigestZero(declared_witness) || DigestZero(receipt->composition_trace_fingerprint)) {
        return LAPLACE_UNIVERSAL_AST_PACKET_INVALID;
    }
    ...
    std::array<std::uint8_t, 32> previous_provider{};
    static_cast<void>(previous_provider);
    ...
    try {
        bindings.reserve(static_cast<std::size_t>(binding_count));
    ...
    for (std::uint64_t index = 0u; index < binding_count; ++index) {
        ...
        if (!CoordinateValid(
            ...
            binding.syntax_span_index == 0u ||
            (index != 0u && binding.syntax_span_index <= prior_span) ||
            (binding.parent_binding_index != NoBinding &&
             binding.parent_binding_index >= index)) {
            return LAPLACE_UNIVERSAL_AST_PACKET_INVALID;
        }
        prior_span = binding.syntax_span_index;

        if (binding.disposition == LAPLACE_AST_RULE_PRESERVE) {
            if (!CoordinateValid(
                    binding.role_coordinate, LAPLACE_HIGHWAY_KIND_AST_ROLE)) {
                return LAPLACE_UNIVERSAL_AST_PACKET_INVALID;
            }
            ++actual_preserved;
        } else if (binding.disposition == LAPLACE_AST_RULE_DECLARED_LOSS) {
            if (!CoordinateZero(binding.role_coordinate)) {
                return LAPLACE_UNIVERSAL_AST_PACKET_INVALID;
            }
            ++actual_loss;
        } else {
        ...
        if (binding.has_content != 0u) {
            ...
            if (!ReferenceZero(binding.content_reference) ||
                binding.byte_start != binding.byte_end ||
    ...
    for (const auto& binding : bindings) HashWitnessBinding(witness, binding);
