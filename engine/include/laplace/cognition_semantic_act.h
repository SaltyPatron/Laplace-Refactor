...
enum {
    ...
    LAPLACE_COGNITION_SEMANTIC_ACT_PRODUCER_OPERATION_PRESENT = UINT32_C(2),
    LAPLACE_COGNITION_SEMANTIC_ACT_KIND_PRESENT = UINT32_C(4),
...
typedef enum laplace_cognition_semantic_act_status {
    ...
    LAPLACE_COGNITION_SEMANTIC_ACT_NO_PRODUCER_OPERATION = 4,
 ...
 *
 * The operation that satisfies the final cognition obligation and the semantic
 * act published after completion are deliberately distinct. `producer_operation_kind`
 * is the final operation selected by the native cognition scheduler (for example,
 * INDEXED_SEARCH). `act_kind` is the semantic result made available to realization;
 * this observation-result boundary publishes ANSWER only after cognition is complete
 * and a retained answer set exists. No prompt bytes, language keywords, endpoint
 * names, or transport-specific intent labels participate in either selection.
 *
 * `primary_answer` is the lowest-rank retained terminal answer while
 * `answer_set_fingerprint` binds every retained answer so realization cannot
 * silently discard or replace the completed result set.
 */
typedef struct laplace_cognition_semantic_act {
    ...
    uint32_t act_kind;
    uint32_t producer_operation_kind;
    uint32_t flags;
    uint32_t version;
 ...
 * semantic completion with zero remaining required obligations. The producer
 ...
 * manufacture completion, or reinterpret the result with a text intent table.
 */
