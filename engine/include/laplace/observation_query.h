...
enum {
    ...
    /*
     * Candidate source planes are deliberately orthogonal to relation family.
     * A predecessor derived from an exact trajectory and one supported by
     * testimony may name the same endpoints while retaining different evidence
     * meaning.  Keep each plane as a single bit so receipts can preserve the
     * contributing provider classes instead of flattening them into adjacency.
     */
    LAPLACE_OBSERVATION_QUERY_SOURCE_PHYSICALITY = UINT32_C(1),
    LAPLACE_OBSERVATION_QUERY_SOURCE_TESTIMONY = UINT32_C(2),
    LAPLACE_OBSERVATION_QUERY_SOURCE_CALCULATION = UINT32_C(4),
    LAPLACE_OBSERVATION_QUERY_SOURCE_GEOMETRY = UINT32_C(8),
    LAPLACE_OBSERVATION_QUERY_SOURCE_STANDING = UINT32_C(16),
    LAPLACE_OBSERVATION_QUERY_SOURCE_LAYER_MASK =
        ...
        LAPLACE_OBSERVATION_QUERY_SOURCE_GEOMETRY |
        LAPLACE_OBSERVATION_QUERY_SOURCE_STANDING,
