...
| `world.admit-witnesses` | Exact source profiles and heterogeneous AST world admission | #53, #59 | Partial: generic tabular recipe plus synthetic and real ISO 639-3 source-to-PostgreSQL whole routes are controlled-integration proven; selected tables are exact while the archive wrapper is provenance-only; 18,805 join-key carriers remain explicitly unresolved; product world admission is absent | Typed ISO Highway topology, unrelated grammars/modalities, cross-profile closure, activated-product readback, complete configured profiles, performance, and seed receipt |
