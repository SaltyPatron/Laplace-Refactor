...
| `evidence.record-lineage` | Immutable occurrences, testimony, derivation, and dependence roots | #16 | Partial: typed evidence-node identity, direct dependence DAG validation, exact cycle paths, root closure/nonamplification, ISA 1.5, and transactional PostgreSQL parity/replay are implemented; complete testimony/admission fields and cross-source integration remain | Cross-source provenance and copied-evidence nonamplification |
