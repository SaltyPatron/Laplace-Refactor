...
def main() -> None:
    # The two source axes are independent of representation/modality.
    p = Path("engine/src/tabular_source.cpp")
    text = p.read_text(encoding="utf-8")
    old = '''    const std::uint32_t epistemic_class =
...
'''
    new = '''    const std::uint32_t epistemic_class =
...
'''
    if text.count(old) != 1:
        raise SystemExit("tabular_source.cpp: dynamic-profile flag anchor changed")
...
'''
    if text.count(old) != 1:
        raise SystemExit("tabular_source.cpp: artifact-graph identity anchor changed")
    text = text.replace(old, new)

    old = "        if (!ValidateReferenceRules() || !ValidateMappingRules()) {\n"
    new = '''        if (LAPLACE_SOURCE_PROFILE_GET_EPISTEMIC_CLASS(
...
'''
    if text.count(old) != 1:
        raise SystemExit("tabular_source.cpp: PlanBuilder validation anchor changed")
    p.write_text(text.replace(old, new), encoding="utf-8", newline="\n")
...
''',
    ...
    for old, new in replacements:
        count = text.count(old)
        if count != 1:
            raise SystemExit(f"source_admission_pg.c: expected one anchor, found {count}: {old[:100]!r}")
        text = text.replace(old, new)
    shifts = [
    ...
    for old, new in shifts:
        count = text.count(old)
        if count != 1:
            raise SystemExit(f"source_admission_pg.c: field shift {old!r} count={count}")
        text = text.replace(old, new)
    p.write_text(text, encoding="utf-8", newline="\n")

    replace_one(
        "tests/postgres/source_admission_contract.sql",
    ...
    for prefix, sql_file, expected_artifacts in (
        ...
        if text.count(old) != 1:
            raise SystemExit(f"{sql_file}: profile authority insert changed")
        ...
        text, count = pattern.subn(
            lambda m: m.group(0) + f" decode(:'{prefix}_artifact_{m.group(1)}_media_type', 'hex'),",
            text,
        )
        if count != expected_artifacts:
            raise SystemExit(f"{sql_file}: media values {count} != {expected_artifacts}")
        ...
        if text.count(old) != 1:
            raise SystemExit(f"{sql_file}: profile flag projection changed")
