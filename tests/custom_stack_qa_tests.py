...
SPEC = importlib.util.spec_from_file_location("laplace_custom_stack_qa_tests", MODULE_PATH)
...
class CustomStackQaTests(unittest.TestCase):
    ...
    def plan(self, *paths: str) -> dict:
        return qa.build_plan(self.contract, ROOT, list(paths), "candidate-sha")

    def test_unrelated_contract_change_keeps_expensive_boundaries_out_of_core(self) -> None:
        ...
        isolated = {row["ctest_name"] for row in self.contract["isolated_tests"]}
    ...
    def test_composition_change_selects_only_composition_boundary(self) -> None:
        ...
        self.assertEqual(plan["selected_profiles"], ["composition-boundary"])
        self.assertEqual(
    ...
    def test_generic_source_admission_change_selects_real_source_regressions(self) -> None:
    ...
    def test_missing_isolated_registry_test_fails_closed(self) -> None:
        ...
        broken["isolated_tests"][0]["ctest_name"] = "invented.expensive.test"
        with self.assertRaisesRegex(qa.QaError, "absent from custom-stack registry"):
    ...
    def test_first_terminal_lane_failure_stops_before_selected_physical_tests(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
...
""" % str(invocations),
            ...
            self.assertEqual(recorded["aggregate_required_qa"], "failed")
            self.assertEqual(recorded["primary_terminal_result"]["lane"], "core")
            self.assertEqual(
                recorded["primary_terminal_result"]["failure"]["test"], "core.fail"
            )
