...
TEST(TabularRecursiveMerge, RetainsWitnessMetadataBoundToCanonicalContent) {
    ...
    laplace::internal::RecursiveDecompositionWitnessInput root{};
    root.provider_fingerprint = Fingerprint(0x41u);
    root.media_type = MediaType;
    root.byte_start = 0u;
    root.byte_end = 2u;
    root.parent_span_index = std::numeric_limits<std::uint64_t>::max();
    root.kind = 0x101u;
    root.media_type_byte_count = sizeof(MediaType) - 1u;
    root.depth = 0u;
    root.flags = 0u;
    ...
    constexpr std::uint64_t DestinationRequestCount = 8u;
    constexpr std::uint64_t CanonicalArtifactResult = 4u;
    ASSERT_EQ(
    ...
    ASSERT_EQ(media_types.size(), sizeof(MediaType) - 1u);
    EXPECT_TRUE(DigestEqual(witnesses[0].trace_fingerprint, trace));
    EXPECT_TRUE(DigestEqual(
        witnesses[0].provider_fingerprint, root.provider_fingerprint));
    ...
    EXPECT_EQ(witnesses[0].canonical_content.reference_kind,
              LAPLACE_COMPOSITION_REFERENCE_PRIOR_RESULT);
    EXPECT_EQ(witnesses[0].canonical_content.reference_index,
              CanonicalArtifactResult);
