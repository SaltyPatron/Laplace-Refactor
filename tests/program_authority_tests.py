...
def validate_continuation(document: dict, verify_physical: bool = True) -> None:
    ...
    require(work.get("state") == "published-implementation-acceptance-pending-merge", "composition implementation proof state drift")
    ...
    contains_all(work.get("implemented_and_locally_proven_boundary", []), ("direct-native", "provider-independent", "cancellation", "blind-presence", "five-sample", "329062"), "issue 15 implementation acceptance was narrowed")
    contains_all(work.get("remaining_acceptance_boundary", []), ("CI", "merge", "500000", "30-seconds-per-GB", "issue 72"), "issue 15 publication and whole-product acceptance boundary was hidden")
    contains_all(work.get("nonclaims", []), ("merged", "performance gates", "not activated", "not seeded", "not released"), "composition implementation was promoted beyond evidence")
...
def validate_operation(document: dict) -> None:
    ...
    contains_all(stages["substrate.bulk-deposit"].get("implementation", {}), ("pull request 73", "semantic parity", "provider-independent", "six provider mutants", "five-sample", "500000-records-per-second", "product activation"), "composition proof state drift")
