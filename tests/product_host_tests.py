...
class CognitionOwnerTests(unittest.TestCase):
            ...
            for omit_policy in (False, True):
                with self.subTest(scope=scope, omit_policy=omit_policy):
                    ...
                    for installed, source in layout.items():
                        content = (REPOSITORY / source).read_text()
                        if omit_policy and installed in (
                                "laplace-openai-api", "laplace-openai-api-transport"):
                            # Reproduce the installed419 defect by removing just
                            # the new loader policy as well as the unit environment.
                            start = content.index("    # Installed package bytes are immutable.")
                            end = content.index("    return module", start)
                            content = content[:start] + "    loader.exec_module(module)\n" + content[end:]
                        (binary / installed).write_text(content)
    ...
    def test_direct_installed_api_launch_cannot_write_bytecode_without_unit_environment(self):
        import hashlib
        release = self.root / "direct-api-package"
        binary = release / "bin"
        binary.mkdir(parents=True)
        layout = {
            "laplace-openai-api": "tools/openai_api_compat.py",
        ...
        def snapshot():
            return {str(path.relative_to(release)): hashlib.sha256(path.read_bytes()).hexdigest()
