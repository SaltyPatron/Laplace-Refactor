...
TEST(WorldAdmission, SemanticProjectionExcludesOnlyPhysicalExecutionReceipts) {
    ...
    EXPECT_EQ(rejected.status, LAPLACE_WORLD_ADMISSION_IDENTITY_MISMATCH);
    const laplace_digest256 empty_digest{};
    EXPECT_EQ(std::memcmp(rejected.semantic_fingerprint.bytes, empty_digest.bytes, 32u), 0);
}
